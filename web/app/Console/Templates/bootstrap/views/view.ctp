...
echo "<?php\n";
$baseCssJs = 'prefix-'.strtolower($pluralHumanName) . '.ver';
...
<?php
	...
	echo $pre1 . "if(in_array(\$userAuth['role'], array('Administrativo', 'Profesor'))) : ?>" . $pre2;
?>
		<li><?php
			echo "<?php";
			echo $pre2 . "echo \$this->Html->link(";
...
<?php
	...
	echo $pre1 . "if(\$userAuth['role'] == 'Administrativo') : ?>" . $pre1;
?>
		<li><?php
			echo "<?php";
	...
	foreach ($associations as $type => $data) :
		foreach ($data as $alias => $details) {
			if ($details['controller'] != $this->name && !in_array($details['controller'], $done)) {
				...
				echo $pre1 . "<?php";
				echo $pre1 . "/* Agregar */";
				echo $pre1 . "if(in_array(\$userAuth['role'], array('Administrativo', 'Profesor'))) : ?>";
				echo $pre2 . "<li><?php";
				echo $pre3 . "echo \$this->Html->link(";
...
<div class="<?php echo $pluralVar; ?> view col-md-9">

<div class="widget">
	<div class="widget-header">
		<i class="icon-th-large"></i>
		<h3><?php echo $singularHumanName; ?></h3>
	</div>
	...
	if (!empty($associations['belongsTo'])) {
		foreach ($associations['belongsTo'] as $alias => $details) {
			if ($field === $details['foreignKey']) {
				...
				echo $pre3 . "<td>";
				echo $pre4 . "<?php echo \$this->Html->link(\${$singularVar}['{$alias}']['{$details['displayField']}'], array('controller' => '{$details['controller']}', 'action' => 'ver', \${$singularVar}['{$alias}']['{$details['primaryKey']}'])); ?>";
				echo $pre3 . "</td>";
				echo $pre2 . "</tr>";
				break;
			}
